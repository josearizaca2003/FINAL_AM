...
import 'package:front_flutter_api_rest/src/pages/sub_categoria_crud/sub_category_create_page.dart';
import 'package:front_flutter_api_rest/src/pages/sub_categoria_crud/sub_category_edit_page.dart';
import 'package:front_flutter_api_rest/src/pages/sub_categoria_crud/sub_category_show_page.dart';
...
class SubCategorialistPage extends StatefulWidget {
  @override
  _SubCategorialistPageState createState() => _SubCategorialistPageState();
}
...
class _SubCategorialistPageState extends State<SubCategorialistPage> {
  ...
  Future<void> _getData({String? nombre}) async {
    try {
      final subCategoriaData =
          await subCategoriaController.getDataSubCategoria(nombre: nombre);
      setState(() {
        item = subCategoriaData
  ...
  Future<void> _removeSubCategoria(int id, String fotoURL) async {
    final response =
        await subCategoriaController.removeSubCategoria(id, fotoURL);

    if (response.statusCode == 200 || response.statusCode == 204) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('sub Categoría eliminada con éxito')),
      );
      _getData();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(
                'Error al eliminar la sub categoría: tiene elementos relacionados.')),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al eliminar la sub categoría.')),
      );
    }
  }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return ConfirmDeleteDialog(
          id: id,
          fotoURL: fotoURL,
          onConfirmDelete: _removeSubCategoria,
        );
      },
    );
  }
              ...
              child: Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    labelText: 'Buscar sub categoría',
              ...
              child: Row(
                ...
                children: [
                  Container(
                    width: 160,
                    child: Text(
                      '¡Bienvenido a la gestión de sub categorías!',
                      style: TextStyle(
                        color: Colors.white,
                        fontSize: 15,
            ...
            item.isEmpty
                ...
                : Column(
                    children: item.map<Widget>((subcategoria) {
                      return Dismissible(
                        key: Key(subcategoria.id.toString()), // Clave única
                        ...
                        onDismissed: (direction) {
                          _removeSubCategoria(
                              subcategoria.id!, subcategoria.foto!);
                        },
                        child: Column(
                          children: [
                            Container(
